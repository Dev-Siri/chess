...
func TestFenValidate(t *testing.T) {
	for _, testFen := range testFens {
		err := chess.ValidateFen(testFen.Fen)

		if testFen.ExpectsErr {
			if err == nil {
				t.Fatalf("TestFenValidate(%s) = %v, expected for `error`", testFen.Fen, err)
			}
		} else {
			if err != nil {
				t.Fatalf("TestFenValidate(%s) = %v, expected for `<nil>`", testFen.Fen, err)
			}
		}
	}
}
